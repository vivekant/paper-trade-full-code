...
const initialState = {
    ...
    watchlist1: [
        { name: "ADANIENT", current_price: 3000, daychange: 100, open_price: 3000, exchange: "NSE" },
        { name: "ADANIENT", current_price: 3000, daychange: 100, open_price: 3000, exchange: "BSE" },

    ],
    watchlist2: [
        { name: "TATAMOTORS", current_price: 610, open_price:600, daychange: 100, exchange: "NSE" },
        { name: "TATAMOTORS", current_price: 580, open_price:590,daychange: 100, exchange: "BSE" },
    ],
    watchlist3: [],
    watchlist4: [],
    watchlist5: [],
...
export const counterSlice = createSlice({
    ...
    reducers: {
        ...
        updatevalue: (state, action) => {
            state.watchlist1.map((item) => {




                // Generate a random multiplier between 0.9 and 1.1 with 2 decimal precision
                const randomNumber = parseFloat((0.95 + (Math.random()) * 0.1).toFixed(2));
                

                // Update item properties with the random multiplier
                item.current_price = parseFloat((item.open_price * randomNumber).toFixed(2));
                item.daychange = parseFloat((item.open_price-item.current_price).toFixed(2));
        ...
        AddToWatchlist: (state, action) => {
            const { watchlistNumber, name, current_price, daychange, exchange,open_price } = action.payload;

            const entity = { name, current_price, daychange, exchange,open_price };
...
export const { AddToWatchlist, AddToPosition, RemoveFromWatchlist, exitPosition, updatePosition, updatedarkmode, updatevalue } = counterSlice.actions
